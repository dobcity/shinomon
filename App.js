...
export default function App() {
  // Определение прав пользователя ВКонтакте
  const getIsAdmin = () => {
    if (typeof window === 'undefined') return true;
    const urlParams = new URLSearchParams(window.location.search);
    const role = urlParams.get('vk_viewer_group_role');
    
    // Если роль не передана (при прямом тестировании вне ВК), показываем админку
    if (!role) return true;

    // Доступ разрешен только администраторам, редакторам и владельцам группы
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* НАВИГАЦИОННЫЕ ВКЛАДКИ (Показываются ТОЛЬКО администраторам) */}
      {isAdmin && (
      ...
      {/* ИСТОРИЯ (Показывается ТОЛЬКО администраторам) */}
      ...
      {/* НАСТРОЙКИ (Показываются ТОЛЬКО администраторам) */}
